...
# =====================================================================@000541=
# 1. podnaloga
# <br/><b>Brez (pa ne zgoraj, ampak spredaj in zadaj)</b> Napiši
# funkcijo `brez(nekNiz)`, ki sestavi nov niz, pri katerem spustimo prvi in
# zadnji znak niza `nekNiz`.
# =============================================================================

# =====================================================================@000547=
# 2. podnaloga
# <br/><b>Brezštevno</b> Napiši
# funkcijo `brezŠtevk(nekNiz)`, ki sestavi nov niz, pri katerem spustimo vse števke, ki nastopajo
# v nizu `nekNiz`. Ali je v spremenljivki `nekZnak` števka, lahko preverimo z
# 
...
# =====================================================================@000548=
# 3. podnaloga
# <br/><b>Brez sredine</b> Napiši
# funkcijo `brezSredine(nekNiz)`, ki nizu spusti srednji znak. Če pa je niz sode dolžine, naj
# ohrani vse znake 
# 
...
def _validate_current_file():
    ...
    if Check.part():
        try:
            Check.equal("brezŠtevk('Matija')", 'Matija')
            Check.equal("brezŠtevk('Triglav 2864m')", 'Triglav m')
            Check.equal("brezŠtevk('1 2 3 4 5')", '    ')
            Check.equal("brezŠtevk('To12in34to')", 'Tointo')
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])
    ...
    if Check.part():
        try:
            Check.equal("brezSredine('Matija')", 'Matija')
            Check.equal("brezSredine('12345')", '1245')
            Check.equal("brezSredine('123')", '13')
            Check.equal("brezSredine('1')", '')
            Check.equal("brezSredine('12')", '12')
            Check.equal("brezSredine('')", '')
            Check.equal("brezSredine('123456')", '123456')
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])
    ...
    try:
        ...
        token = 'Token f9dbf58689fac42d32daf0367c5dc1d4acc5dd8b'
